...
if [ ! -f "${HOME}/.codex/auth.json" ]; then
  echo "Missing ${HOME}/.codex/auth.json (required for Codex auth)." >&2
  exit 1
fi

if [ ! -d "${HOME}/.codex/skills" ]; then
  echo "Missing ${HOME}/.codex/skills (required for Codex skills)." >&2
  exit 1
fi

export HARNESS_API_TOKEN="${HARNESS_API_TOKEN:-dev-token}"
export HARNESS_RUN_CMD_TEMPLATE="${HARNESS_RUN_CMD_TEMPLATE:-codex exec {prompt}}"
...
response="$(curl -sS -m 5 -w '\n%{http_code}' -X POST \
  ...
  -d '{"prompt":"codex integration test: say hello"}' \
...
echo "Codex integration OK: ${host_stdout_path}"
