...
- A formal schema and filtering rules documented in `docs/contracts/schemas/auditd.filtered.v1.md` and `docs/contracts/collector_config/audit_filtering.md`.
...
This phase also introduced a dedicated CLI test suite (`scripts/cli_scripts`) and updated docs (`docs/contracts/install.md`, `docs/contracts/cli.md`, `lasso/README.md`) to reflect the CLI-first workflow.

**Phase 18: Documentation restructure + user guide bundling (Feb 4, 2026)**
Docs were reorganized into a dedicated `docs/` tree with a user-first guide (`docs/contracts/`) and a developer section (`docs/dev/`). The documentation map moved to `docs/README.md`, and the root `README.md` was simplified to point users to the guide while keeping a developer pointer. The release bundle was updated to ship `docs/contracts/` for offline access, and the config reference was expanded (including a clear explanation of `HARNESS_API_TOKEN`).
...
**Phase 21: Run-scoped logging layout + timeline copies (Feb 12, 2026)**
Logging moved from flat `/logs/*` paths to run-first directories created at
`lasso up` time (`lasso__YYYY_MM_DD_HH_MM_SS`). Collector outputs now live under
`collector/raw` and `collector/filtered`, harness artifacts under
`harness/sessions` and `harness/jobs`, and labels under `harness/labels`.

This phase also added one filtered timeline copy per completed session/job so
raw scoped evidence can be inspected directly without the UI.

CLI behavior was updated so:
- `lasso up` fails fast if already running,
- `logs`/`jobs` default to active run,
- historical inspection is supported via `--run-id` and `--latest`.

Agent immutability hardening was restored in compose (`cap_drop: SYS_ADMIN`,
