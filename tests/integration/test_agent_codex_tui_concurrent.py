...
def _session_metrics(rows: list[dict[str, Any]], session_id: str) -> dict[str, Any]:
    ...
    for row in scoped:
        if row.get("event_type") != "fs_write":
            continue
        details = row.get("details")
        if isinstance(details, dict) and isinstance(details.get("path"), str):
            fs_write_paths.append(details["path"])
    ...
    return {
        ...
        "fs_write_count": len(fs_write_paths),
        "fs_write_paths": fs_write_paths,
...
def _session_meets_thresholds(
    ...
    *,
    ...
    return (
        ...
        and metrics["fs_write_count"] >= 1
        and metrics["meaningful_net_count"] >= 1
        and metrics["unique_pid_count"] >= 3
        and expected_path in metrics["fs_write_paths"]
    )
...
def test_codex_tui_concurrent_kalshi_and_nhl_lanes(
    ...
    try:
        ...
        timeline_rows = codex_stack.wait_for_timeline_rows(
            lambda rows: _session_meets_thresholds(
                rows, session_id=kalshi_session_id, expected_path=expected_kalshi_path
            )
            and _session_meets_thresholds(
                rows, session_id=nhl_session_id, expected_path=expected_nhl_path
            ),
            timeout_sec=120,
            message=(
                "Concurrent TUI lanes did not reach minimum timeline thresholds "
    ...
    assert kalshi_metrics["fs_write_count"] >= 1, f"Kalshi lane fs_write threshold not met: {kalshi_metrics}"
    assert nhl_metrics["fs_write_count"] >= 1, f"NHL lane fs_write threshold not met: {nhl_metrics}"
    ...
    assert f"/work/{kalshi_file}" in kalshi_metrics["fs_write_paths"], (
        "Kalshi expected fs_write path missing.\n"
        f"metrics={kalshi_metrics}"
    )
    assert f"/work/{nhl_file}" in nhl_metrics["fs_write_paths"], (
        "NHL expected fs_write path missing.\n"
        f"metrics={nhl_metrics}"
    )
