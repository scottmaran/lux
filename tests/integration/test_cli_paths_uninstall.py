...
def test_uninstall_remove_data_flag_is_rejected(tmp_path: Path, lasso_cli_binary: Path) -> None:
    home = tmp_path / "home"
    ...
    result = _run(
        [str(lasso_cli_binary), "uninstall", "--remove-data"],
        cwd=ROOT_DIR,
        env=env,
        timeout=30,
        check=False,
...
def test_uninstall_dry_run_preserves_files_and_never_targets_data_dirs(
    ...
    with serve_directory(server_root) as base_url:
        ...
        (home / "lasso-logs").mkdir(parents=True, exist_ok=True)
        (home / "lasso-workspace").mkdir(parents=True, exist_ok=True)
        ...
        env_file.write_text("LASSO_VERSION=v0.1.0\n", encoding="utf-8")
        ...
        assert str(home / "lasso-logs") not in planned
        assert str(home / "lasso-workspace") not in planned

        # Dry run must not mutate filesystem.
        assert (home / ".lasso" / "versions" / "0.1.0").exists()
        assert (home / ".lasso" / "current").exists()
        assert lasso.exists()
        assert (config_dir / "config.yaml").exists()
        assert env_file.exists()
        assert (home / "lasso-logs").exists()
        assert (home / "lasso-workspace").exists()
...
def test_uninstall_succeeds_with_invalid_config_and_without_env_file(
    ...
    with serve_directory(server_root) as base_url:
        ...
        assert not lasso.exists()
        assert not (home / ".lasso").exists() or not (home / ".lasso" / "versions").exists()
