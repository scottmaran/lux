...
- Integration/regression/stress now assert behavior only from live stack outputs
  (`filtered_audit.jsonl`, `filtered_ebpf.jsonl`, `filtered_timeline.jsonl`)
  generated by submitted jobs/sessions.
- Offline synthetic replay is intentionally not used as acceptance evidence in
  those layers.
...
This means fixture/unit synthetic inputs remain essential for deterministic
contract coverage, while live-stack layers validate runtime behavior.
...
Synthetic raw logs are currently used in unit/fixture fidelity and contract
tests, not as integration acceptance evidence.

Synthetic audit lines remain minimally viable today and intentionally shorter
than full production audit output, but the target remains production-shape
fidelity.

## Near-Term Direction

Near-term target is to move from minimal synthetic raw logs to **fully verbose,
production-shape synthetic logs** that are as close as possible to real
audit/eBPF output while preserving determinism.
...
- Builder realism gap on eBPF side:
  - builder coverage now includes configured event types, but payload richness
    still needs to expand to mirror more production variants.
...
1. Enrich existing eBPF builders with additional realistic payload variants:
   - multiple protocol/family combinations,
   - syscall_result edge cases,
   - richer dns/unix payload diversity.
...
3. Continue synthetic fidelity tests:
...
4. Keep fixtures and builders aligned:
...
- Increase eBPF builder payload richness beyond baseline key coverage.
- Expand synthetic fidelity assertions as new real sample patterns are observed.
- Keep integration acceptance tests live-stack only; reserve synthetic builders for unit/fixture scope.
